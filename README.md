# ebekkers.github.io

This site has moved to <https://ideal-machine-intelligence.eu/>.

The repository now serves only a redirect: `index.html` for the root and
`404.html` so that every other path under `ebekkers.github.io` redirects too.
`.nojekyll` keeps GitHub Pages from running Jekyll over these files.

The previous al-folio Jekyll site is preserved on the `jekyll-site-archive`
branch (and in this branch's history, up to commit 0004670).
